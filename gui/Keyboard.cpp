...
void Keyboard::render(Element* parent)
{
	...
	CST_SetDrawColorRGBA(parent->renderer, 0xf9, 0xf9, 0xf9, 0xFF);
	...
	for (int y = 0; y < 3; y++)
		for (int x = 0; x < 10 - y - (y == 2); x++)
		{
			...
			CST_SetDrawColorRGBA(parent->renderer, 0xf4, 0xf4, 0xf4, 0xff);
	...
	if (curRow >= 0 || index >= 0)
	{
		...
		if (touchMode)
		{
			CST_SetDrawColorRGBA(parent->renderer, 0xad, 0xd8, 0xe6, 0x90); // TODO: matches the DEEP_HIGHLIGHT color
			CST_FillRect(parent->renderer, &dimens2);
		}
		else
		{
			...
			for (int z = 4; z >= 0; z--)
			{
				CST_SetDrawColorRGBA(parent->renderer, 0x66 - z * 10, 0x7c + z * 20, 0x89 + z * 10, 0xFF);
				dimens2.x--;
				dimens2.y--;
				dimens2.w += 2;
				dimens2.h += 2;
	...
	//   CST_Rect dimens3 = {this->x+dPos, this->y + dHeight, dWidth, textSize};
	//   CST_SetDrawColorRGBA(parent->renderer, 0xff, 0xaa, 0xaa, 0xff);
